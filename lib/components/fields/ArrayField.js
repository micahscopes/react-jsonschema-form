...
exports.ArrayFieldTitle = ArrayFieldTitle;
exports.ArrayFieldDescription = ArrayFieldDescription;
...
var ArrayField = function (_Component) {
  ...
  function ArrayField() {
    ...
    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref3 = ArrayField.__proto__ || Object.getPrototypeOf(ArrayField)).call.apply(_ref3, [this].concat(args))), _this), _this.onAddClick = function (event) {
    ...
    }, _this.onReorder = function (index, newIndex) {
      var move = function move(from, to, fromIndex, toIndex) {
        var shift = toIndex < fromIndex ? -1 : 1;
        for (var i in from) {
          i = Number(i);
          if (i == fromIndex) {
            to[toIndex] = from[fromIndex];
          } else if (i < Math.min(fromIndex, toIndex) || i > Math.max(toIndex, fromIndex)) {
            to[i] = from[i];
          } else if (shift > 0) {
            to[i - 1] = from[i];
          } else if (shift < 0) {
      ...
      var newFormData = [];
      move(formData, newFormData, index, newIndex), console.log("moving ARRAY!!!!", index, newIndex, formData, newFormData);
      ...
      return function (event) {
        ...
        _this.onReorder(index, newIndex);
      };
